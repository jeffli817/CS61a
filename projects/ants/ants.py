...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive, min_range=0, max_range=float('inf')):
        ...
        placeCurrent = self.place
        range =0
        while placeCurrent != beehive and (min_range<=range<=max_range):
            if len(placeCurrent.bees) >0:
                return rANTdom_else_none(placeCurrent.bees)
            placeCurrent = placeCurrent.entrance
            range +=1
        return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor<= amount:
            atk = self.damage + amount
        else:
            atk = amount
        lstCopy = self.place.bees[:]
        for bee in lstCopy:
            Insect.reduce_armor(bee, atk)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting>0:
            ...
            HungryAnt.eat_bee(self, rANTdom_else_none(self.place.bees))
        # END Problem 6
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.isTrue== False:
            ...
            copylst = self.place
            while copylst.exit != None:
                if copylst.exit.ant!= None and copylst.exit.ant not in self.buffed:
                    copylst.exit.ant.damage *=2
                    self.buffed.append(copylst.exit.ant)
                copylst = copylst.exit
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if self.isTrue == True and amount>= self.armor:
            bees_win()
        else:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        if self.direction == 'backward' and self.place.entrance.name !='Hive':
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem Optional 4
        if target and target.direction == 'forward':
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        dict = {}
        while temp_place != beehive:
            if (temp_place.ant != None and i != 0):
                dict[temp_place.ant] = i
            if (len(temp_place.bees) >0):
