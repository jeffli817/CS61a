...
def swine_align(player_score, opponent_score):
    ...
    if player_score==0 or opponent_score == 0:
        ...
        minimum = min(player_score,opponent_score)
        maximum = max(player_score,opponent_score)
        d = minimum
        while d >=10:
            if maximum % d == 0 and minimum % d ==0:
                return True
            d = d-1
        return False
    # END PROBLEM 4a
...
def pig_pass(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4b
    "*** YOUR CODE HERE ***"
    if opponent_score > player_score and opponent_score - player_score <3 :
        return True
    return False
    # END PROBLEM 4b
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0<goal and score1<goal:
        ...
        if who==0:
            ...
            score0 += take_turn(num_dice, score1, dice)
            if extra_turn(score0, score1):
                extra = 1
        else:
            num_dice =  strategy1(score1, score0)
            score1 += take_turn(num_dice, score0, dice)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    highest_dice = 0
    while x >0:
            result = average(x, dice)
            if result >= current_highest_score:
                highest_dice = x
                current_highest_score = result
            x -=1
    return highest_dice
