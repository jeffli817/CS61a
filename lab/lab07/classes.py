...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        for x in player.deck.cards:
            x.attack +=other_card.attack
            x.defense += other_card.defense
        lst=[]
        for idx,y in enumerate(opponent.deck.cards):
            if y.attack == other_card.attack or y.defense ==other_card.defense:
                lst.append(idx)
        for x in reversed(lst):
            opponent.deck.cards.pop(x)
