...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        operands_val = [x.eval(env) for x in self.operands]
        return operator_val.apply(operands_val)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        "*** YOUR CODE HERE ***"
        envCopy = self.parent.copy()
        x = dict(zip(self.parameters,arguments))
        for y in x:
            envCopy[y] = x[y]

        return self.body.eval(envCopy)
