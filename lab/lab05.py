...
def distance(city_a, city_b):
    ...
    "*** YOUR CODE HERE ***"
    return sqrt(
    (get_lon(city_a)-get_lon(city_b))**2 + (get_lat(city_a)-get_lat(city_b))**2
    )
...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [deck[idx//2] if idx%2==0 else deck[(len(deck)//2)+(idx-1)//2] for idx, val in enumerate(deck)]
...
def add_trees(t1, t2):
    ...
    for i in range(max(len1,len2)):
        if i < min(len1,len2):
            temp.append(add_trees(branch1[i],branch2[i]))
        elif i>=len1:
            temp.append(branch2[i])
        else:
            temp.append(branch1[i])
